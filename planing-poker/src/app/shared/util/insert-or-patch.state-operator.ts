...
import { insertItem, NoInfer, patch, updateItem } from '@ngxs/store/operators';
...
export function insertOrPatch<T>(searchFn: (a: T, b: T) => boolean, arr: T[], keepOld = false): StateOperator<T[]> {
  return (arrState: Readonly<T[]>) => {
    return arr.reduce((acc: T[], curr) => {
        ...
        if (i !== -1) {
          return updateItem<T>(u => searchFn(u as T, curr), patch(curr) as NoInfer<T>)(acc as RepairType<T>[]) as T[];
        } else {
          return insertItem<T>(curr as NoInfer<T>)(acc as RepairType<T>[]) as T[];
        }
      },
